...
class LiteLLMAPI(ChatAPI):
    """LiteLLM API for unified access to 100+ LLM providers.

    Usage: Pass model string like "litellm/openai/gpt-4o" or "litellm/anthropic/claude-3-5-sonnet"
    The "litellm/" prefix will be stripped before routing to LiteLLM.
    Note: gpt-5 models don't support temperature parameter.
    """

    def __init__(self, model: str):
        # Strip "litellm/" prefix - rest goes to LiteLLM (e.g., "openai/gpt-5")
        if model.startswith("litellm/"):
            self.model = model[8:]  # Remove "litellm/" prefix
        else:
            self.model = model
